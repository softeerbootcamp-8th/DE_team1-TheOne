...
- **운행**: 월별 택시 운행 단위 실데이터 (`main/spark/jobs/bronze_to_silver/monthly_taxi_trip_bronze_to_silver/transformer.py` — 거리/시간/driver_pay/tips/zone/platform, `taxi_id`·`driver_id`는 매핑으로 채워짐)
- **기사**: `driver_master` (성향 — 주요 거리대·시간대·요일, 근무/휴식/공차 min-max, joined_at/churned_at)
- **회사 원장**: `customer` / `taxi` / `lease_contract` (`sub/generators/synthetic_company_snapshot/snapshot.py` — 차량, 주간 리스료, 등급 자격, 계약일)
