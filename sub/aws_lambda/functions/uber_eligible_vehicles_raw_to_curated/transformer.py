...
Raw 한 행은 "이 차종은 2018년식부터 Comfort, Comfort Electric 가능" 처럼
**상품이 여러 개 묶인** 형태입니다. 그대로 두면 쓰는 쪽이 매번 리스트를
풀어야 하고, "이 차로 Comfort 를 받을 수 있나?" 를 조인 조건으로 쓸 수 없습니다.

그래서 Curated 는 **(차종, 상품) 하나당 한 행**으로 펼칩니다.

    Raw      ZDX  "2010 (UberX, Comfort) / 2018 (Comfort Electric)"
    Curated  ZDX  UberX             min_year=2010
...
class UberEligibleVehiclesCuratedTransformer(Transformer):
    """Raw 배차 가능 목록을 (차종, 상품) 단위 Curated 행으로 펼칩니다."""

    def transform(self, data: list[dict]) -> list[dict]:
        if not data:
            raise ValueError("변환할 Uber 배차 가능 목록 Raw 데이터가 없습니다.")
        ...
        logger.info("curated_transform done rows=%d", len(curated))
        return curated
