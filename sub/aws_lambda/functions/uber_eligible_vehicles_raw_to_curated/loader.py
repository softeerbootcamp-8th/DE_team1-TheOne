"""정제된 Uber 배차 가능 차량 목록을 Curated Parquet 으로 적재합니다."""
...
def _group_by_city(data: list[dict]) -> dict[str, list[dict]]:
    if not data:
        raise ValueError("적재할 Uber 배차 가능 목록 Curated 데이터가 없습니다.")
...
class UberEligibleVehiclesCuratedLoader(Loader):
...
class UberEligibleVehiclesS3CuratedLoader(Loader):
...
def build_curated_loader(
    ...
    if storage == "local":
        return UberEligibleVehiclesCuratedLoader(base_dir, expect_collected_date=collected_date)
    if storage == "s3":
        return UberEligibleVehiclesS3CuratedLoader(expect_collected_date=collected_date, bucket=bucket)
