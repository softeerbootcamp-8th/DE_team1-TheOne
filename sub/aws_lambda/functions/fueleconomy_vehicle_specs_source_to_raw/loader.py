...
class VehicleSpecsRawLoader(Loader):
    ...
    def write(self, data: list[dict]) -> WriteResult:
        ...
        path = layout.raw_file(self._base_dir, source, self._collected_at)
        ...
        logger.info(
            "raw_load done path=%s rows=%d columns=%d bytes=%d",
            path,
            table.num_rows,
            table.num_columns,
            path.stat().st_size,
...
class VehicleSpecsS3RawLoader(Loader):
    ...
    def write(self, data: list[dict]) -> WriteResult:
        source = data[0][PARTITION_KEY]
        key = layout.raw_key(source, self._collected_at)
        ...
        logger.info(
            "raw_load done location=%s rows=%d columns=%d bytes=%d",
            result.location,
            table.num_rows,
            table.num_columns,
            len(body),
...
def build_raw_loader(
    ...
    if storage == "local":
        return VehicleSpecsRawLoader(base_dir, collected_at)
    if storage == "s3":
        return VehicleSpecsS3RawLoader(collected_at, bucket=bucket)
