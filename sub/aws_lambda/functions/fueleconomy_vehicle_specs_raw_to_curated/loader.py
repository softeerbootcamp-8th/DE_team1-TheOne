"""정제된 차종별 제원을 Curated Parquet 으로 적재합니다."""
...
def _group_by_source(data: list[dict]) -> dict[str, list[dict]]:
    if not data:
        raise ValueError("적재할 차종별 제원 Curated 데이터가 없습니다.")
...
class VehicleSpecsCuratedLoader(Loader):
    ...
    def write(self, data: list[dict]) -> WriteResult:
        ...
        for source, source_rows in sorted(by_source.items()):
            ...
            path = layout.curated_file(self._base_dir, collected_date, source)
            ...
            logger.info(
                "curated_load done path=%s source=%s rows=%d",
                path,
                source,
                table.num_rows,
            )
...
class VehicleSpecsS3CuratedLoader(Loader):
    """출처별 Curated Parquet 하나씩 S3에 씁니다. 같은 key는 재실행 시 덮어씁니다."""
    ...
    def write(self, data: list[dict]) -> WriteResult:
        ...
        for source, source_rows in sorted(by_source.items()):
            ...
            key = layout.curated_key(collected_date, source)
            table = pa.Table.from_pylist(source_rows, schema=SCHEMA)
            buffer = io.BytesIO()
            ...
            logger.info(
                "curated_load done location=%s source=%s rows=%d",
                result.location,
                source,
                table.num_rows,
            )
...
def build_curated_loader(
    ...
    if storage == "local":
        return VehicleSpecsCuratedLoader(base_dir, expect_collected_date=collected_date)
    if storage == "s3":
        return VehicleSpecsS3CuratedLoader(expect_collected_date=collected_date, bucket=bucket)
