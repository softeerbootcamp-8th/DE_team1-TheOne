...
# Curated 를 만드는 데 실제로 필요한 컬럼만 읽습니다. Raw 는 원본 84컬럼
# 5만 행이라 전부 dict 로 펼치면 수백 MB 를 씁니다. 나머지 컬럼은 버리는 게
# 아니라 Raw 에 그대로 남아 있고, 필요해지면 여기에 이름만 추가하면 됩니다.
NEEDED_COLUMNS = (
    "id",
    "year",
    "make",
...
class VehicleSpecsRawExtractor(Extractor):
    """해당 collected_date 파티션에서 출처별 가장 최신 Parquet 을 읽어 합칩니다.

    Raw 는 collected_date 아래에 source 파티션이 한 단계 더 있습니다.
    매 실행이 전량 스냅샷이라 같은 날 여러 번 돌면 파일이 쌓이므로
    출처별로 최신 것만 씁니다.
    """

    name = "fueleconomy_vehicle_specs_raw"
    ...
    def extract(self) -> list[dict]:
        ...
        for source_dir in source_dirs:
            ...
            if not paths:
                raise FileNotFoundError(f"Raw Parquet 파일이 없습니다: {source_dir}")
            ...
            try:
                ...
                if missing:
                    raise RuntimeError(
                        f"Raw 에 필요한 컬럼이 없습니다: {missing} ({path})"
                    )
        ...
        logger.info(
            "raw_extract done sources=%d rows=%d", len(source_dirs), len(rows)
        )
        return rows


class VehicleSpecsS3RawExtractor(Extractor):
    """S3 Raw에서 해당 날짜 파티션의 출처별 최신 Parquet을 읽어 합칩니다."""
    ...
    def extract(self) -> list[dict]:
        ...
        for source, source_keys in sorted(by_source.items()):
            ...
            try:
                ...
                if missing:
                    raise RuntimeError(
                        f"Raw 에 필요한 컬럼이 없습니다: {missing} (s3://{self._bucket}/{key})"
                    )
                table = parquet.read(columns=list(NEEDED_COLUMNS))
            except (OSError, pa.ArrowInvalid) as exc:
                raise RuntimeError(
        ...
        logger.info("raw_extract done sources=%d rows=%d", len(by_source), len(rows))
        return rows
...
def build_raw_extractor(
    ...
    if storage == "local":
        return VehicleSpecsRawExtractor(base_dir, collected_date)
    if storage == "s3":
        return VehicleSpecsS3RawExtractor(collected_date, bucket=bucket)
