...
Raw 는 원본 CSV 를 그대로 실은 것이라 **값이 전부 문자열**이고 컬럼이 84개입니다.
Curated 에서 쓰는 컬럼만 골라 숫자로 바꾸고 조인 키를 붙입니다.
...
Curated 는 원본 레코드 하나당 한 행을 유지하고 `source_id` 로 되짚게 합니다.
...
class VehicleSpecsCuratedTransformer(Transformer):
    """Raw 제원을 조인 가능한 Curated 행 목록으로 정제합니다."""

    def transform(self, data: list[dict]) -> list[dict]:
        if not data:
            raise ValueError("변환할 차종별 제원 Raw 데이터가 없습니다.")
        ...
        if not curated:
            raise ValueError("차종별 제원 Curated 변환 결과가 0건입니다.")
        ...
        collected_dates = {row["collected_at"].date() for row in curated}
        if len(collected_dates) != 1:
            raise ValueError("하나의 Raw 스냅샷에 수집일이 섞여 있습니다.")

        logger.info("curated_transform done rows=%d skipped=%d", len(curated), len(skipped))
        return curated
