...
class UberEligibleVehiclesRawLoader(Loader):
...
class UberEligibleVehiclesS3RawLoader(Loader):
...
def build_raw_loader(
    ...
    if storage == "local":
        return UberEligibleVehiclesRawLoader(base_dir, city_slug, collected_at)
    if storage == "s3":
        return UberEligibleVehiclesS3RawLoader(city_slug, collected_at, bucket=bucket)
