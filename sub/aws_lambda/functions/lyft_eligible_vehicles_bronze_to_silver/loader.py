...
from shared.aws_lambda.common import lyft_eligible_vehicles_layout as layout
...
class LyftEligibleVehiclesS3SilverLoader(Loader):
...
def build_silver_loader(
    ...
    if storage == "local":
        return LyftEligibleVehiclesSilverLoader(base_dir, expect_collected_date=collected_date)
    if storage == "s3":
        return LyftEligibleVehiclesS3SilverLoader(expect_collected_date=collected_date, bucket=bucket)
