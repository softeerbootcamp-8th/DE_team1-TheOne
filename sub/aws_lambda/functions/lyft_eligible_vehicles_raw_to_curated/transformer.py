"""Lyft 배차 가능 차량을 차량 대장과 조인 가능한 Curated 행으로 정제합니다.

Raw 입력은 make, model, min_year, products, collected_at을 사용합니다.
products 예상값은 Extra Comfort, XL, XXL, Black, Black SUV이며,
Extractor가 파티션의 city와 원본 위치인 bronze_path를 추가합니다.
Curated에서는 make/model 조인 키와 상품별 최소 허용 연식으로 사용합니다.
"""
...
class LyftEligibleVehiclesCuratedTransformer(Transformer):
    """Raw 차량 목록을 (차종, 상품) 하나당 한 행으로 펼칩니다."""

    def transform(self, data: list[dict]) -> list[dict]:
        if not data:
            raise ValueError("변환할 Lyft 배차 가능 목록 Raw 데이터가 없습니다.")
