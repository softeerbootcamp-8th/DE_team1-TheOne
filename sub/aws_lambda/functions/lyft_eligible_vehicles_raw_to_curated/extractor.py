"""실행일의 Lyft 배차 가능 차량 Raw 스냅샷을 읽습니다."""
...
class LyftEligibleVehiclesRawExtractor(Extractor):
    """해당 날짜 파티션에서 도시별 최신 Raw Parquet을 읽습니다."""

    name = "lyft_eligible_vehicles_raw"
...
class LyftEligibleVehiclesS3RawExtractor(Extractor):
...
def build_raw_extractor(
    ...
    if storage == "local":
        return LyftEligibleVehiclesRawExtractor(base_dir, collected_date)
    if storage == "s3":
        return LyftEligibleVehiclesS3RawExtractor(collected_date, bucket=bucket)
