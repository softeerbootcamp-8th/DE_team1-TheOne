...
from schema.source import VEHICLE_CATALOG_BRONZE_SCHEMA as SCHEMA
...
class VehicleCatalogRawLoader(Loader):
    ...
    def write(self, data: list[dict]) -> WriteResult:
        path = layout.raw_file(self._base_dir, data[0]["vendor"], self._collected_at)
...
class VehicleCatalogS3RawLoader(Loader):
    ...
    def write(self, data: list[dict]) -> WriteResult:
        key = layout.raw_key(data[0]["vendor"], self._collected_at)
...
def build_raw_loader(
    ...
    if storage == "local":
        return VehicleCatalogRawLoader(base_dir, collected_at)
    if storage == "s3":
        return VehicleCatalogS3RawLoader(collected_at, bucket=bucket)
