"""Lyft Premium 대상 차량 행을 Raw Parquet으로 적재합니다."""
...
from schema.source import LYFT_ELIGIBLE_VEHICLES_SCHEMA as SCHEMA
...
class LyftEligibleVehiclesRawLoader(Loader):
    """추출 결과를 선별하지 않고 일별 Raw 파일 하나로 로컬에 저장합니다."""
    ...
    def write(self, data: list[dict]) -> WriteResult:
        path = layout.raw_file(self._base_dir, self._city_slug, self._collected_at)
        ...
        logger.info(
            "raw_load done path=%s rows=%d bytes=%d",
            path,
            table.num_rows,
            path.stat().st_size,
        )
...
class LyftEligibleVehiclesS3RawLoader(Loader):
    """추출 결과를 선별하지 않고 일별 Raw 파일 하나로 S3에 저장합니다."""
    ...
    def write(self, data: list[dict]) -> WriteResult:
        key = layout.raw_key(self._city_slug, self._collected_at)
        ...
        logger.info(
            "raw_load done location=%s rows=%d bytes=%d",
            result.location,
            table.num_rows,
            len(body),
        )
...
def build_raw_loader(
    ...
    if storage == "local":
        return LyftEligibleVehiclesRawLoader(base_dir, city_slug, collected_at)
    if storage == "s3":
        return LyftEligibleVehiclesS3RawLoader(city_slug, collected_at, bucket=bucket)
