...
Raw 의 차종 표기는 업체 카드 이미지에서 읽은 값이라 전부 대문자입니다
...
class VehicleCatalogCuratedTransformer(Transformer):
    """Raw 차량 대장을 조인 가능한 Curated 행 목록으로 정제합니다."""

    def transform(self, data: list[dict]) -> list[dict]:
        rows = data
        if not rows:
            raise ValueError("변환할 차량 대장 Raw 데이터가 없습니다.")
                ...
                curated.append(
                    {
                        ...
                        "image_url": image_url,
                        # 어느 Raw 파일에서 나왔는지. 같은 날 여러 번 수집하면
        ...
        if errors:
            raise ValueError("차량 대장 Curated 변환 실패:\n- " + "\n- ".join(errors))
