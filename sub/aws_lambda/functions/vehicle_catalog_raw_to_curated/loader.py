"""정제된 리스 업체 보유 차량 대장을 Curated Parquet 으로 적재합니다."""
...
from schema.source import VEHICLE_CATALOG_SCHEMA as SCHEMA
...
def _group_by_vendor(data: list[dict]) -> dict[str, list[dict]]:
    if not data:
        raise ValueError("적재할 차량 대장 Curated 데이터가 없습니다.")
...
class VehicleCatalogCuratedLoader(Loader):
    """업체별로 Parquet 하나씩 씁니다. 같은 파티션은 덮어씁니다.

    Raw 와 달리 Curated 는 재실행하면 덮어씁니다. 같은 수집일을 다시 변환한
    ...
    def write(self, data: list[dict]) -> WriteResult:
        ...
        for vendor, vendor_rows in sorted(by_vendor.items()):
            ...
            path = layout.curated_file(self._base_dir, collected_date, vendor)
            ...
            logger.info(
                "curated_load done path=%s vendor=%s rows=%d",
                path,
                vendor,
                table.num_rows,
            )
...
class VehicleCatalogS3CuratedLoader(Loader):
    """업체별 Curated Parquet 하나씩 S3에 씁니다. 같은 key는 재실행 시 덮어씁니다."""
    ...
    def write(self, data: list[dict]) -> WriteResult:
        ...
        for vendor, vendor_rows in sorted(by_vendor.items()):
            ...
            key = layout.curated_key(collected_date, vendor)
            table = pa.Table.from_pylist(vendor_rows, schema=SCHEMA)
            buffer = io.BytesIO()
            ...
            logger.info(
                "curated_load done location=%s vendor=%s rows=%d",
                result.location,
                vendor,
                table.num_rows,
            )
...
def build_curated_loader(
    ...
    if storage == "local":
        return VehicleCatalogCuratedLoader(base_dir, expect_collected_date=collected_date)
    if storage == "s3":
        return VehicleCatalogS3CuratedLoader(expect_collected_date=collected_date, bucket=bucket)
