"""리스 업체 보유 차량 대장 Raw 를 Curated 로 변환합니다."""
...
from .transformer import VehicleCatalogCuratedTransformer
...
def lambda_handler(event: dict | None = None, context=None) -> dict:
    ...
    result = Pipeline(
        ...
        loader,
        transformer=VehicleCatalogCuratedTransformer(),
    ).run()

    return {
        "row_count": result.write_result.row_count,
        # 업체(vendor)별로 파일 하나씩 씁니다 — 업체 수는 len(locations) 입니다.
