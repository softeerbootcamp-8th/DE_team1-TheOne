"""Lyft Eligible Vehicles Raw -> Curated 전체 Pipeline 검증."""
...
from sub.aws_lambda.functions.lyft_eligible_vehicles_raw_to_curated.handler import (
...
def write_raw(raw_dir: Path, rows: list[dict]) -> str:
    return LyftEligibleVehiclesRawLoader(
        str(raw_dir), CITY, COLLECTED_AT
    ).write(rows).location


def run_curated(raw_dir: Path, curated_dir: Path) -> dict:
    return to_curated(
        event={
...
def test_Handler가_도시별_Curated_Parquet을_적재한다(tmp_path):
    ...
    result = run_curated(raw_dir, curated_dir)

    expected = (
        layout.date_partition(str(curated_dir), COLLECTED_DATE)
...
def test_같은_날짜를_재실행하면_같은_파일을_덮어쓴다(tmp_path):
    raw_dir, curated_dir = tmp_path / "raw", tmp_path / "curated"
    write_raw(raw_dir, [vehicle(2019, ["Black"])])

    first = run_curated(raw_dir, curated_dir)
    second = run_curated(raw_dir, curated_dir)
...
def test_요청일과_Raw_수집일이_다르면_적재하지_않는다(tmp_path):
    raw_dir, curated_dir = tmp_path / "raw", tmp_path / "curated"
    stale = COLLECTED_AT.replace(day=9)
    write_raw(raw_dir, [vehicle(2019, ["Black"], stale)])

    with pytest.raises(ValueError, match="변환된 수집일이 다릅니다"):
        run_curated(raw_dir, curated_dir)
