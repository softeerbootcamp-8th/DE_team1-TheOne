...
def source_to_raw_task(**context) -> dict:
    """Uber 자격 페이지를 수집해 Raw 에 적재합니다."""
    params = context.get("params", {})
    result = lambda_handler_for("uber_eligible_vehicles_source_to_raw", package="sub.aws_lambda.functions")(
        event={
...
def raw_to_curated_task(raw_result: dict, **context) -> dict:
    ...
    result = lambda_handler_for("uber_eligible_vehicles_raw_to_curated", package="sub.aws_lambda.functions")(
        event={
...
def validate_raw_task(result: dict, **context) -> None:
    ...
    loader = importlib.import_module(
        "sub.aws_lambda.functions.uber_eligible_vehicles_source_to_raw.loader"
    )
    transformer = importlib.import_module(
        "sub.aws_lambda.functions.uber_eligible_vehicles_raw_to_curated.transformer"
    )
...
@task(
    ...
    outlets=[assets.UBER_ELIGIBLE_VEHICLES_CURATED],
    retries=1,
    retry_delay=timedelta(minutes=10),
    on_failure_callback=slack_failure_callback,
)
def validate_curated_task(result: dict, **context) -> None:
    ...
    loader = importlib.import_module(
        "sub.aws_lambda.functions.uber_eligible_vehicles_raw_to_curated.loader"
    )
    transformer = importlib.import_module(
        "sub.aws_lambda.functions.uber_eligible_vehicles_raw_to_curated.transformer"
    )
