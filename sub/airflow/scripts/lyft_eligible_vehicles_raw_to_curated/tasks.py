...
def run_gx_bronze_validation(
    ...
    run_gx_validation(
        dataframe,
        expectations,
        suite_name="lyft_eligible_vehicles_bronze_suite",
        layer="bronze",
    )
...
def run_gx_silver_validation(
    ...
    if "product" in dataframe.columns:
        expectations.append(
            gx.expectations.ExpectColumnValuesToBeInSet(
                column="product", value_set=allowed_products
            )
        )
    ...
    run_gx_validation(
        dataframe,
        expectations,
        suite_name="lyft_eligible_vehicles_silver_suite",
        layer="silver",
    )
...
def source_to_raw_task(**context) -> dict:
    ...
    result = invoke_lambda(
        "lyft_eligible_vehicles_source_to_raw",
...
@task(
    ...
    outlets=[assets.LYFT_ELIGIBLE_VEHICLES_CURATED],
    retries=1,
    retry_delay=timedelta(minutes=10),
    on_failure_callback=slack_failure_callback,
)
