...
가장 중요한 건 `collected_date` 를 고르는 규칙입니다. Raw 핸들러는 자기
실행 시각으로 파티션을 정하는데, Curated 쪽이 날짜를 따로 계산하면 자정 근처
실행에서 둘이 어긋나 **없는 파티션을 읽습니다.** 그래서 Raw 가 돌려준 값을
...
DAG_FILE = (
    Path(__file__).resolve().parents[1] / "dags" / "vehicle_catalog_raw_to_curated_dag.py"
)
DAG_ID = "vehicle_catalog_raw_to_curated_pipeline"
RAW_FUNCTION = "vehicle_catalog_source_to_raw"
CURATED_FUNCTION = "vehicle_catalog_raw_to_curated"

# Raw 가 돌려줬다고 가정할 수집일. 아래 PARAM_DATE 와 반드시 달라야
...
def events(dag_module, monkeypatch):
    ...
    def fake_lambda_handler_for(function_name: str, **_kwargs):
        def handler(event: dict) -> dict:
            ...
            # Raw 태스크의 반환값 모양만 맞춰줍니다. Curated 태스크는 이걸 안 씁니다.
            return {"collected_date": RAW_DATE}
...
def silver_event(events: list[tuple[str, dict]]) -> dict:
    """받아 적은 것 중 Curated 핸들러로 간 event 하나를 꺼냅니다."""
    matched = [event for name, event in events if name == CURATED_FUNCTION]
    assert len(matched) == 1, f"Curated 핸들러 호출이 1건이 아닙니다: {events}"
    return matched[0]
...
def test_적재와_검증이_번갈아_이어진다(dag_module):
    ...
    검증이 변환 앞에 있어야 깨진 Raw 를 읽지 않습니다. 이 순서가 뒤집히면
    Curated 가 먼저 돌아 검증이 사후 통보가 됩니다.
    """
...
def test_Bronze_event_는_base_dir_와_collected_date_를_넘긴다(dag_module, events):
    ...
    call_task(dag_module, "source_to_raw", params={"raw_dir": "/tmp/raw"})

    name, event = events[0]
    assert name == RAW_FUNCTION
    assert event == {"base_dir": "/tmp/raw", "collected_date": None}
...
def test_collected_date_가_없으면_Bronze_가_알려준_값을_쓴다(dag_module, events):
    ...
    assert silver_event(events)["collected_date"] == RAW_DATE
...
def test_경로_파라미터가_비면_DAG_기본값을_쓴다(dag_module, events):
    ...
    call_task(dag_module, "source_to_raw", params={})
    call_task(
        dag_module,
        "raw_to_curated",
        raw_result={"collected_date": RAW_DATE},
