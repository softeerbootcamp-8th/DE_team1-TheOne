...
from shared.lambda_runtime.common import uber_eligible_vehicles_layout as layout
...
class UberEligibleVehiclesS3SilverLoader(Loader):
    ...
    def write(self, data: list[dict]) -> WriteResult:
        ...
        for city, city_rows in sorted(by_city.items()):
            ...
            key = layout.silver_key(collected_date, city)
            table = pa.Table.from_pylist(city_rows, schema=SCHEMA)
            buffer = io.BytesIO()
...
def build_silver_loader(
    ...
    if storage == "local":
        return UberEligibleVehiclesSilverLoader(base_dir, expect_collected_date=collected_date)
    if storage == "s3":
        return UberEligibleVehiclesS3SilverLoader(expect_collected_date=collected_date, bucket=bucket)
