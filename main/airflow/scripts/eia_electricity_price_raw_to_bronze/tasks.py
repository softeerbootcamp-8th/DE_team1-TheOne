"""EIA 월간 전력요금 원본 수집 DAG의 실행·검증 함수.

이 파일 하나에 2010년부터의 이력이 통째로 들어 있어 매일 받을 이유가 없습니다.
월 1회로 갱신하는 것은 EIA 가 과거 값을 개정하므로 최신 개정분을 확보하기
위해서입니다. 특히 월간 전력 통계는 발표 후 몇 달간 바뀝니다.
"""
...
def validate_bronze_task(result: dict, **context) -> None:
    ...
    if context["params"].get("dry_run") is True:
        if result.get("dry_run") is not True:
            raise ValueError("EIA 전력 Bronze dry-run 결과 표시가 없습니다")
        location = parsed.locations[0]
        if isinstance(location, S3Location):
            if location.key != layout.electricity_bronze_key(collected_date):
        ...
        if (
            ...
            or size < layout.ELECTRICITY_MIN_BYTES
        ):
    ...
    if isinstance(path, S3Location):
        if path.key != layout.electricity_bronze_key(collected_date):
