"""EIA 휘발유 요금 Bronze→Silver DAG 계약. 이슈 #517.

1. 월간 스케줄과 세 단계 순서 — 원본 확인이 변환보다 먼저
2. 지정이 없으면 직전 달을 고름. 휘발유 주간 계열은 약 1주 지연이라 전력(3개월)과 다름
...
def test_DAG는_월간_스케줄로_확인_변환_검증을_순서대로_처리한다():
    assert DAG.dag_id == "eia_gas_price_bronze_to_silver_pipeline"
...
def test_원본이_없으면_돌려야_할_DAG_를_알려주며_실패한다(tmp_path):
    with pytest.raises(FileNotFoundError, match="eia_gas_price_raw_to_bronze_pipeline"):
...
def test_일수부족_중복일자_스키마불일치는_실패한다(tmp_path, violation):
    ...
    if violation == "missing_day":
        ...
        _write(path, [{"date": date(2024, 3, d), "price": 3.4} for d in range(1, 32)], schema)
