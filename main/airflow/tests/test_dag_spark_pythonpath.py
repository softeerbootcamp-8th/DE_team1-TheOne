...
DAG 목록을 하드코딩하지 않고 `bash_command` 가 `main/spark/jobs/` 를 가리키는 태스크를
...
REQUIRED_ROOTS = ("", "/main/spark", "/libs/pipeline_core")
    ...
    for dag_path in sorted(DAGS_DIR.glob("*.py")):
        ...
        for value in vars(module).values():
            ...
            for task in value.tasks:
                ...
                if command and "main/spark/jobs/" in command:
...
def test_Spark_job_을_부르는_태스크를_실제로_찾았다():
    ...
    assert SPARK_TASKS, "main/spark/jobs/ 를 부르는 BashOperator 를 하나도 못 찾았습니다"
