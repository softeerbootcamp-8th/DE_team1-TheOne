...
def test_대상연월은_기준일_이하_최신_HVFHV_파티션이다(tmp_path):
    for year_month in ("2026-03", "2026-05", "2026-09"):
        partition = tmp_path / "hvfhv" / f"year_month={year_month}"
        partition.mkdir(parents=True)
...
def test_Silver_4종이_있으면_같은_월_경로를_확정한다(tmp_path):
    ...
    assert resolved["hvfhv_path"].endswith(
        "hvfhv/year_month=2026-05/part-*.parquet"
    )
...
def test_API_Silver는_가장최신_collected_at_파일만_선택한다(tmp_path):
    ...
    assert Path(resolved["hvfhv_path"]).name == latest
    assert Path(resolved["driver_snapshot_path"]).name == latest
    assert Path(resolved["inventory_path"]).name == latest
