...
산출물은 `eia_gas_price/year_month=YYYY-MM/eia_gas_price.parquet` 입니다. 충전 단가
CLEAN Silver 와 함께 연료비 통합 단계의 입력이 됩니다.

스케줄
-----
지정이 없으면 직전 달을 채웁니다. 휘발유 주간 계열은 약 1주 지연이라 직전 달이면 그
달 전 주가 이미 나와 있습니다 — 전력(약 3개월 지연)과 다른 점입니다. 수집 DAG 가 매월
1일 05시에 돌고 나면 그 결과로 한 달을 만드는 흐름이라 그 뒤에 둡니다.
...
@dag(
    dag_id="eia_gas_price_bronze_to_silver_pipeline",
    default_args=default_args,
    # 휘발유 수집(05시)이 끝난 뒤에 돕니다.
    schedule="0 7 1 * *",
...
def eia_gas_price_bronze_to_silver_pipeline():
...
eia_gas_price_bronze_to_silver_dag = eia_gas_price_bronze_to_silver_pipeline()
