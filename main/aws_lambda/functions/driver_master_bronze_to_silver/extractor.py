"""기사 계약 Bronze 원본 Parquet 한 파일을 읽습니다."""
...
class DriverVehicleLeaseBronzeExtractor(Extractor):
    """월 파티션의 원본 파일을 그대로 읽습니다. 정제는 Transformer 가 합니다."""

    name = "driver_vehicle_leases_bronze"
    ...
    def extract(self) -> pa.Table:
        if not self._path.is_file():
            raise FileNotFoundError(f"기사 계약 Bronze 파일이 없습니다: {self._path}")
        try:
            table = pq.ParquetFile(self._path).read()
        except (OSError, pa.ArrowInvalid) as exc:
            raise ValueError(
                f"기사 계약 Bronze가 읽을 수 있는 Parquet이 아닙니다: {self._path}"
            ) from exc
