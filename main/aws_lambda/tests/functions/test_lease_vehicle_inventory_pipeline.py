    ...
    return {
        ...
        "datasets": {
            "lease_vehicle_inventory": {
                ...
                "download_url": (
                    f"/v1/data/{YEAR_MONTH}/datasets/lease_vehicle_inventory"
                ),
            },
...
def _api(monkeypatch, manifest: dict, requested: list[str] | None = None):
    responses = {
        ...
        f"{API_URL}/v1/data/{YEAR_MONTH}/datasets/lease_vehicle_inventory": Response(
            content=CONTENT
        ),
    }
...
def test_보유차량만_원본bytes그대로_Bronze에_저장한다(tmp_path, monkeypatch):
    requested = []
...
def test_manifest에_보유차량dataset이_없으면_다운로드하지않는다(tmp_path, monkeypatch):
    manifest = _manifest()
    del manifest["datasets"]["lease_vehicle_inventory"]
    requested = []
...
def test_checksum이_다르면_완료파일을_공개하지않는다(tmp_path, monkeypatch):
    manifest = _manifest()
    manifest["datasets"]["lease_vehicle_inventory"]["sha256"] = "0" * 64
