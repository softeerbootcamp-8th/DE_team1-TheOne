...
기사 순수익 증가(threshold)를 여러 값으로 스윕해 매번 재배정하고, 값마다 별도
`threshold`로 태그한 행을 쌓는다. threshold가 0보다 크므로 "현재 차량 유지"보다
못한(순수익이 줄어드는) 배정은 적격 필터에서 걸러져 나올 수 없다 — 기존 기사가
차량 교체로 손해를 보는 경우는 생기지 않는다.
"""
...
class RevenueFirstAlgorithm(VehicleRecommendationAlgorithm):
    ...
    def recommend(self, driver_metrics: DataFrame, inventory: DataFrame) -> DataFrame:
        ...
        outputs = [
            self._recommend_for_threshold(candidates, threshold)
            for threshold in self._thresholds
        ]
        candidates.unpersist()
        return reduce(DataFrame.unionByName, outputs)

    def _recommend_for_threshold(
        self, candidates: DataFrame, threshold: int
    ) -> DataFrame:
        ...
        eligible = candidates.filter(
            ...
            | (
                ...
                & (F.col("expected_net_profit_increase") >= threshold)
            )
        )
        ...
        return _finalize_recommendation_output(
            allocated, self.ALGORITHM_VERSION_ID, threshold
        )
