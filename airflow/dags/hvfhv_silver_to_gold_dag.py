"""월별 Gold 3종(기사 집계·차량 추천·월간 리포트) 산출과 검증 DAG.
...
REQUIRED_COLUMNS = {
    "driver_aggregation": {"driver_id", "year_month", "monthly_net_profit", "monthly_rental_fee"},
...
@dag(
    dag_id="hvfhv_silver_to_gold_pipeline", default_args=default_args,
    # 배정 DAG 가 매월 12일 01:00 이라 그 뒤에 둡니다.
    schedule="0 3 13 * *", start_date=datetime(2024, 1, 1), catchup=False,
    max_active_runs=1, tags=["hvfhv", "gold", "spark"],
    params={
...
def silver_to_gold_pipeline():
    ...
    def validate_inputs(**context) -> dict:
    ...
    build = BashOperator(
        task_id="build_gold",
        bash_command=(
            f"python {ROOT}/spark/jobs/silver_to_gold/job.py "
