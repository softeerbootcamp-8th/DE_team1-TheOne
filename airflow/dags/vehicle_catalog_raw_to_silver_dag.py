...
def _failure_column(expectation_config) -> str:
    column = expectation_config.kwargs.get("column")
    if column:
        return str(column)
    if expectation_config.type == "expect_compound_columns_to_be_unique":
        return "/".join(expectation_config.kwargs["column_list"])
    return "table"


def _run_gx_validation(dataframe, expectations, *, layer: str) -> None:
    """차량 대장 품질 규칙을 실행하고 실패 내용을 Airflow 로그로 남깁니다."""
    ...
    batch = (
        context.data_sources.add_pandas(name=f"vehicle_catalog_{layer}_source")
        .add_dataframe_asset(name=f"vehicle_catalog_{layer}_asset")
        .add_batch_definition_whole_dataframe(f"vehicle_catalog_{layer}_batch")
        .get_batch(batch_parameters={"dataframe": dataframe})
    )
    validation = batch.validate(
        gx.ExpectationSuite(
            name=f"vehicle_catalog_{layer}_suite",
    ...
    for failure in failures:
        ...
        logger.error(
            ...
            _failure_column(failure.expectation_config),
    ...
    if failures:
        rules = ", ".join(
            ...
            f"[{_failure_column(failure.expectation_config)}]"
            for failure in failures
        )
        raise ValueError(f"Vehicle Catalog {layer.title()} GX 검증 실패: {rules}")
