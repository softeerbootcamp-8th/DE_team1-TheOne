"""EV Charging Bronze -> Silver DAG의 2컬럼 적재·GX 계약을 검증합니다."""
...
def row(target_date: date = date(2026, 7, 1), **overrides) -> dict:
    values = {"date": target_date, "ev_price": 0.31}
...
def test_정상_2컬럼_silver는_검증을_통과한다(silver_dir):
    path = write_silver(
        silver_dir,
        [row(), row(date(2026, 7, 2), ev_price=0.42)],
    )
...
def test_arrow_물리_스키마가_loader와_다르면_거부한다(silver_dir):
    broken_schema = loader.SCHEMA.set(
        loader.SCHEMA.get_field_index("ev_price"),
        pa.field("ev_price", pa.float64(), nullable=False),
    )
