...
from schema.silver.gas_ev_price import SCHEMA
...
PARTITION_KEY = "collected_month"
FILE_NAME = "gas_ev_price.parquet"


def silver_file(base_dir: str, collected_month: str) -> Path:
    return Path(base_dir) / DATASET / f"{PARTITION_KEY}={collected_month}" / FILE_NAME
...
class EiaFuelPriceSilverLoader(Loader):
    ...
    def write(self, data: list[dict]) -> WriteResult:
        ...
        path = silver_file(self._base_dir, self._collected_month)
        ...
        logger.info(
            ...
            path, self._collected_month, table.num_rows,
        )
