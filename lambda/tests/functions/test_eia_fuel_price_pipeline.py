...
10. Bronze 는 대상 월 이하 최신 수집분을 고르고, 없으면 가장 오래된 것으로 물러섬
...
def test_월간_전력요금은_뉴욕_교통부문만_읽는다():
    ...
    # CA(99.0)는 걸러지고 NY 두 달만 남습니다.
    assert parsed == {"2025-05": 20.0, "2025-06": 21.0}


def test_일별_전개는_그날_이하_최근_관측치를_복제한다():
    rows = build_daily_prices("2025-05", _gas_xls(GAS), _electricity_xlsx(ELECTRICITY))
...
def test_전기_단가는_전력요금에_마진_배수를_곱한다(markup):
    rows = build_daily_prices(
        "2025-05", _gas_xls(GAS), _electricity_xlsx(ELECTRICITY), markup=markup
    )
...
def test_대상월_전_일수를_채운다():
    ...
    # 하루라도 비면 combine_price_tables 가 "날짜 집합이 다릅니다" 로 죽고,
    # 통과하더라도 Gold 의 일자 조인에서 그 날 운행이 통째로 매칭 실패합니다.
...
def test_원본_시작일보다_이른_달은_실패한다():
    with pytest.raises(ValueError, match="이전의 휘발유 관측치가 없습니다"):
        build_daily_prices("2025-03", _gas_xls(GAS), _electricity_xlsx(ELECTRICITY))


def test_전력_이력에_없는_달은_보유기간을_알려주며_실패한다():
    with pytest.raises(ValueError, match="보유 2025-05 ~ 2025-06"):
        build_daily_prices("2025-07", _gas_xls(GAS), _electricity_xlsx(ELECTRICITY))
...
def test_가격이_허용범위_밖이면_실패한다(gas_price, electricity_cents, expected):
    ...
    with pytest.raises(ValueError, match=expected):
        build_daily_prices("2025-05", gas, electricity)
...
def test_bronze에서_silver까지_통합_스키마로_적재한다(tmp_path):
    ...
    # `pq.read_table` 은 경로의 `collected_month=` 를 파티션 컬럼으로 덧붙이므로
...
def test_대상월_이하_최신_수집분을_고르고_없으면_가장_오래된것을_쓴다(tmp_path):
    bronze = tmp_path / "bronze"
    # 이력 파일이라 나중에 받은 것에도 과거 값이 들어 있습니다. 그래도 대상 월 이하를
    # 우선하는 것은 그 사이 EIA 가 개정한 값이 섞이는 것을 막기 위해서입니다.
    _write_bronze(bronze, date(2025, 5, 10), _gas_xls(GAS), _electricity_xlsx(ELECTRICITY))
    _write_bronze(bronze, date(2026, 8, 17), _gas_xls(GAS), _electricity_xlsx(ELECTRICITY))

    within = layout.latest_bronze_partition(str(bronze), layout.GAS_DATASET, date(2025, 5, 28))
    assert within.name == "collected_date=2025-05-10"

    # 대상 월이 첫 수집보다 과거면 `as_of` 이하가 없으므로 가장 오래된 것으로 물러섭니다.
    fallback = layout.latest_bronze_partition(str(bronze), layout.GAS_DATASET, date(2024, 1, 28))
    assert fallback.name == "collected_date=2025-05-10"
