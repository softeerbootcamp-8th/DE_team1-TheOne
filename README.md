...
리스 업체의 고객 담당자(CSM)는 수백 명의 기사를 관리하지만, 어떤 고객이 차량을 변경했을 때 기사의 순수익과 리스 업체의 매출이 동시에 개선되는지 판단하기 어렵습니다. 기사의 수익을 높이면서 더 리스 비용이 비싼 차량을 맞춤 추천할 수 있다면 리스 업체 역시 더 많은 매출을 올릴 수 있습니다.

본 시스템은 실제 승차공유 운행 기록에서 고객별 주간 수익을 계산하고, 렌탈 시세 및 차량 제원 데이터와 교체 시 손익을 비교하여 **매주/매달 우선적으로 연락할 고객 20명과 맞춤 제안 차량**을 산출합니다.

### 🎯 핵심 요약 (Summary)

- **대상 (Who)**: 뉴욕 Uber·Lyft 기사 대상 차량 리스 업체의 고객 담당자
- **문제점 (Problem)**: 기사에게 더 높은 순수익을 주면서도 높은 매출을 올릴 수 있는 차량을 데이터 기반으로 추천하지 못해 매출 증대 기회 상실
- **해결 방안 (Solution)**: 차량 변경 시 **'기사 주간 순수익 증가 & 리스 업체 매출액(렌탈료) 상승'** 조건을 충족하는 상위 20명과 제안 차량을 대시보드로 제공하여 담당자가 즉시 추천 영업에 활용하도록 지원
- **핵심 지표 (Key Metric)**: 예상 매출액, 기사별 주간 순수익 증가액 ($/주)
...
| 구분 | Before | After |
| --- | --- | --- |
| **차량 추천 방식** | 판단 기준 부재로 감에 의존한 수동/비효율 관리 | 주행 데이터 기반 객관적 순수익·매출 상승 시뮬레이션 추천 |
| **고객 케어 우선순위** | 수백 명의 기사 중 누구에게 먼저 제안할지 불명확 | 주간 순수익 증가 폭이 가장 큰 **상위 20명 자동 추출** |
| **비즈니스 효과** | 차량 업그레이드 추천 기회 누락으로 매출 정체 | 기사 순수익 극대화 만족도 증대 & **리스 업체 매출액 증대** |
