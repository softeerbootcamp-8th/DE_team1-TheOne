...
# streamlit 은 `streamlit run` 시 이 스크립트를 __name__ == "__main__" 모듈로 실행한다.
# 이 가드가 없으면 `import app`(pytest 등) 만으로도 아래 렌더링·실데이터 로드가 통째로
# 실행돼, data/gold 에 이 대시보드가 기대하는 gold_mart_* 산출물이 없는 환경(예: CI)에서
# 임포트 자체가 깨진다.
